...
describe('api test Queries', () => {
  ...
  beforeEach(() => {
    req = {
      body: {
        name: 'User',
        email: '63ef3ab2c17e649f9388412f',
        content: 'well'
      }
    };
    res = {
  ...
  it('return a 201 status to create query', async () => {
    ...
    const createdQuery = await request(app).post('/api/query/get').send(query);
...
describe('GET /api/blogs/get', () => {
  ...
  it('return a 201 status to create blog', async () => {
    ...
    const createdBlog = await request(app).post('/api/blogs/').send(blog);
    expect(createdBlog.status).toEqual(201);
    expect(createdBlog.body.data).toHaveProperty('topic', 'title', 'content', 'likes', 'comments', '_id');
  });
  it('return a 201 status to update blog ', async () => {
    ...
    expect(updatedBlog.body.data).toHaveProperty('title', 'content', 'likes', 'comments', '_id');
  });
